...
class Orders_model extends TI_Model {
    ...
    public function addOrder($order_info = array(), $cart_contents = array()) {
        ...
        if (isset($order_info['order_time'])) {
            $current_time = time();
            $order_time = (strtotime($order_info['order_time']) < $current_time) ? date('Y-m-d H:i:s', $current_time) : $order_info['order_time']; 
    ...
    public function getMailData($order_id) {
        ...
        if ($result) {
            ...
            $data['assigneeId']=$result['assignee_id'];
//   assgn id and name added startt  
            //assign id and name added
            //$rsl=$this->db->where('name', $data['assigneeId']);
            
        // $query = $this->db->query("SELECT * FROM hhhway_staffs WHERE staff_id=$result['assignee_id'];");
        //  $rsut = $this->db->get_where('hhhway_staffs', array('staff_id' => $result['assignee_id'],1));
            $sqqq = 'select * from hhhway_staffs where staff_id='.$result['assignee_id'];

            $rslt = $this->db->query($sqqq)->row_array();

            $data['staff_name']=$rslt['staff_name'];
//assign id and name added end
//window pick up code start

            $data["code"]=$rslt['password'];
//window pick up code end
