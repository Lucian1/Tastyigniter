...
//////////////
	...
	private function _saveStaff($staff_email, $username,$id) {
        if ($this->validateForm($staff_email, $username) === TRUE) {
            ...
            if ($staff_id = $this->Staffs_model->saveStaff($this->input->get('id'), $this->input->post())) {
				...
				// $sql_select = "SELECT * from hhhway_staffs where staff_name=".$uname;
				...
				// 	// 	);
				// 	// $this->db->where('staff_name', $uname);
				// 	// $this->db->update('hhhway_staffs', $data2);
					   
				// 				   // $this->db->where('staff_name', $staff_name_exist);
				// 				// 	$sql_update="UPDATE `hhhway_staffs` SET `password` = ? 
				// 				// 	WHERE `hhhway_staffs`.`staff_name` = ?";
				// 				//    $this->db->query($sql_update,$data);	   
				//  }
				//
			// else{
				// $sqqq1 = 'INSERT INTO hhhway_window (staff_id,window_num,window_PIN)
				// VALUES (11,'.$this->input->post('username').','.$this->input->post('password').');';
				// if(count<=0)
				// {
			//   $staff_id
					$staff_info = $this->Staffs_model->getStaff((int) $this->input->get('id'));
					$selc_sql = "SELECT * FROM `hhhway_staffs` where staff_id =".$staff_info['staff_id'];
					$bool=$this->db->query($selc_sql);
					...
					// 	$this->load->database();
					// 	$data3 = array();
					// 	$data3['password'] = '111111';
					// 	$ret = $this->db->from("hhhway_staffs")
					// ->where('staff_id',$staff_info['staff_id'])
					// ->set($data3['password'])->update();
						$update_sql="UPDATE `hhhway_staffs` SET `password` =".$pass." WHERE `hhhway_staffs`.`staff_id` =".$staff_info['staff_id'];
				...
				$sqqq="INSERT INTO `hhhway_staffs` (`staff_name`, `staff_email`, `password`) 
				VALUES (?, ?, ?)";
				 $this->db->query($sqqq,$data1);
			}
				

				// $sqqq = "INSERT INTO hhhway_window (staff_id,window_num,window_PIN)
				// VALUES (12,?,00011)";
				// $sqqq="INSERT INTO `hhhway_window` (`window_id`, `staff_id`, `window_num`, `window_PIN`) 
				// VALUES (NULL, ?, ?, ?)";
				...
				//$this->db->insert('hhhway_users', $data1);
